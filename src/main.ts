...
program
    ...
    .option(
        '-t, --trim-from [block num]',
        'Trim blocks in db from [block num] onwards',
        undefined
    )
    ...
    .option(
        '-p, --gaps-purge',
        'In case db integrity check fails purge db from last valid block',
        false
    )
    .option(
        '-S, --skip-start-block-check',
        'Skip initial get_block query to configured endpoint',
        false
    )
    .option(
        '-r, --skip-remote-check',
        'Skip initial get_info query to configured remoteEndpoint',
        false
    )
    .action(async (options) => {
        ...
        if (options.onlyDBCheck) conf.runtime.onlyDBCheck = options.onlyDBCheck;

        if (options.gapsPurge) conf.runtime.gapsPurge = options.gapsPurge;
        ...
        if (options.reindexInto) conf.runtime.reindexInto = options.reindexInto;

        if (options.trimFrom)
            conf.runtime.trimFrom = options.trimFrom
                ? parseInt(options.trimFrom, 10)
                : undefined;
